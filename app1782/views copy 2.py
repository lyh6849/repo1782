...
def close_above_a_input(request):
  ...
  for i in a_bank.objects.all():
    if i.a_id==a_id_97623:
      lvl=i.a_lvl
      parent=i.a_parent
  return JsonResponse({"a_id":a_id_97623,"note":a_value_92623,"lvl":lvl,"parent":parent})
  ...
  for i in selected:
    for j in a_bank.objects.all():
      if j.a_id==i:
        ...
        lvl=j.a_lvl
        parent=j.a_parent
  return JsonResponse({"a_id":selected,"note":note,"lvl":lvl,"parent":parent})
...
def initial_open(request):
  ...
  for i in a_bank.objects.all().order_by('a_id'):
    target_array_a.append([i.q_id,i.a_id,i.a_value,i.a_note,i.a_lvl,i.a_parent])
...
def add_a_create_new_qc(request):
  ...
  target.save()
  target=a_bank(a_id=request.POST['a_id'], a_class=new_qc+"a"+number_to_3_digits(new_ac_numb), q_id=request.POST['q_id'], a_value=request.POST['a_value'],a_note=request.POST['a_note_914'],a_parent=request.POST['a_parent_914'],a_lvl=request.POST['a_lvl_914'])
  target.save()
  target=a_class(a_class=new_qc+"a"+number_to_3_digits(new_ac_numb), a_value=request.POST['q_id'],q_class=new_qc)
  target.save()
  return JsonResponse({"a_note":request.POST['a_note_914'],"a_parent":request.POST['a_parent_914'],"a_lvl":request.POST['a_lvl_914']})
...
def add_a_apply_exst_qc(request):
  ...
  a_parent=request.POST['a_parent_914']  
  same_class_id=[]
  same_class_pair=[]
  for i in q_bank.objects.all():
  ...
  for i in same_class_id:
    ...
    target=a_bank(a_id=new_a_id_178,a_value=a_value_178,q_id=i,a_class=new_ac(i),a_note=a_note,a_lvl=a_lvl,a_parent=a_parent)
    print(new_ac(i))
    target.save()
    same_class_pair.append([i,new_a_id_178])
  return JsonResponse({'same_class_pair':same_class_pair,"a_value_178":a_value_178,"a_lvl_914":a_lvl,"a_note_914":a_note,"a_parent_914":a_parent})
  ...
  for a in a_bank.objects.all().order_by('a_id'):
    if(len(a.a_id)>=n):
      if (first_n(a.a_id,n)==input_id_7516):
        a_7516.append([parent_id_7516+remove_first_n(a.a_id,n),a.a_value,a.a_class,a.a_note,a.a_lvl,a.a_parent])
...
def a_insert_ajax_2(request):
    print()
    a_db = a_bank(a_id=request.POST['a_id'], a_value=request.POST['a_value'], q_id=request.POST['q_id'],a_class=request.POST['a_class'],a_note=request.POST['a_note'],a_lvl=request.POST['a_lvl'],a_parent=request.POST['a_parent'])
    a_db.save()
    return redirect('/')
  ...
  for r in new_q_id_pair:
    ...
    for q in a_class.objects.all().order_by('a_class'):
      if remove_last_n(q.a_class,4)==r[0]:
        ...
        a_number=a_number+1
        target=a_bank(a_id=new_a_id,a_value=q.a_value,q_id=r[1],a_class=q.a_class,a_note=q.a_value,a_lvl=3,a_parent=remove_last_n(r[1],4))
        target.save()
...
def a_bank_load(request):
  ...
  for i_1714 in a_bank_1714:
    ijeorj=[i_1714.id,i_1714.a_id,i_1714.q_id,i_1714.a_value,i_1714.a_class,i_1714.a_note,i_1714.a_lvl,i_1714.a_parent]
...
def update_parent(request):
  target = get_object_or_404(a_bank, a_id=request.POST['a_id'])
  target.a_parent=request.POST['a_parent']
  target.save()
  return JsonResponse({"completed":request.POST['a_parent']})
...
def add_a_common_save(request):
  ...
  if used_914<2:
    ...
    target.save()
    target=a_bank(a_value=a_value_914,a_id=request.POST['a_id_914'],q_id=q_id_914,a_class=new_ac_914,a_lvl=a_lvl,a_note=a_note,a_parent=a_parent)
    target.save()
  return JsonResponse({"used_914":used_914,"a_lvl_914":a_lvl,"a_note_914":a_note,"a_parent_914":a_parent})
